; given:  foldr  ∈  (CF CF -> CF) CF CF -> CF
; prove:  foldr1 ∈  (CF CF -> CF) (CF & cons?) -> CF
(declare-sort T)
...
(assert
 (forall
  ((f T))
  (=>
   (forall ((x T)) (=> (cf x) (forall ((y T)) (=> (cf y) (cf (app (app f x) y))))))
   (forall ((xs T)) (=> (and (cf xs) (or (= xs unr) (= (app ptr-cons? xs) unr) (= (app ptr-cons? xs) k-True))) (cf (app (app ptr-rec-foldr1 f) xs)))))))
(assert
 (not
  (forall
   ((f T))
   (=>
    (forall ((x T)) (=> (cf x) (forall ((y T)) (=> (cf y) (cf (app (app f x) y))))))
    (forall ((xs T)) (=> (and (cf xs) (or (= xs unr) (= (app ptr-cons? xs) unr) (= (app ptr-cons? xs) k-True))) (cf (app (app ptr-foldr1 f) xs))))))))
(check-sat)